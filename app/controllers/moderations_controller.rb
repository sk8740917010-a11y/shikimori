class ModerationsController < ShikimoriController
  ...
  def clear_cache
    authorize! :restart, Shikimori
    Rails.cache.clear
    redirect_back fallback_location: moderations_url, notice: 'Rails.cache.clear'
  end

  private

  def abuse_requests_stats
    Rails.cache.fetch %i[abuse_requests_stats v5], expires_in: 1.day do
      stats = AbuseRequest.where('created_at > ?', 4.months.ago)
        .where(approver_id: User.where("roles && '{#{Types::User::Roles[:forum_moderator]}}'"))
        .group(:approver_id).count
      
      stats.map do |id, count|
        user = User.find_by(id: id)
        OpenStruct.new(approver: user, count: count) if user
      end.compact.sort_by(&:count).reverse
    end
  end

  def bans_stats
    Rails.cache.fetch %i[bans_stats v5], expires_in: 1.day do
      stats = Ban.where('created_at > ?', 4.months.ago)
        .where(moderator_id: User.where("roles && '{#{Types::User::Roles[:forum_moderator]}}'"))
        .group(:moderator_id).count
        
      stats.map do |id, count|
        user = User.find_by(id: id)
        OpenStruct.new(moderator: user, count: count) if user
      end.compact.sort_by(&:count).reverse
    end
  end

  def content_versions_stats
    Rails.cache.fetch %i[content_versions_stats v5], expires_in: 1.day do
      stats = Version.where('created_at > ?', 4.months.ago)
        .where.not(item_type: AnimeVideo.name)
        .where(moderator_id: User.where("roles && '{#{Types::User::VERSION_ROLES.join(',')}}'"))
        .group(:moderator_id).count
  ...
  def critiques_stats
    Rails.cache.fetch %i[critiques_stats v5], expires_in: 1.day do
      stats = Critique.where('created_at > ?', 4.months.ago)
  ...
  def collections_stats
    Rails.cache.fetch %i[collection_stats v5], expires_in: 1.day do
      stats = Collection.where('created_at > ?', 4.months.ago)
        .where.not(moderation_state: :pending)
        .where.not(approver_id: nil)
        .group(:approver_id).count
  ...
  def articles_stats
    Rails.cache.fetch %i[article_stats v2], expires_in: 1.day do
      stats = Article.where('created_at > ?', 4.months.ago)
        .where.not(moderation_state: :pending)
        .where.not(approver_id: nil)
        .group(:approver_id).count
